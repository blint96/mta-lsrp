...
local alphaMult = 0.64 -- water alpha multiplier
local normalStrength = {0.05,0.05,0.05} -- refraction distortion intensity
local detailRefRect = {150,150} -- refraction material size
...
function enableWaterRef()
	...
	dxSetShaderValue( imgRefractShader, "gNormalStrength", normalStrength )
...
addEventHandler( "onClientHUDRender", root, function()
    ...
    local isNotToRun = (not dBuffAccess and not forceOnIfNoDB)
	dxSetShaderValue( texWaterShader, "bAnimWaves", isNotToRun )
	if not camMatIsProper or isNotToRun then return end
